...
class ARCHITECTURE(nn.Module):
    def __init__(
        ...
        if self.use_spatial:
            self.encoder = SpatialEncoder(
                self.input_dim, self.z_dim, self.layer_dims, gcn_dims, dropout
            )
            self.decoder = Decoder(
                self.input_dim, self.z_dim, self.layer_dims[::-1], dropout
            )
    ...
    def forward(self, x, edge_index=None, edge_weight=None):
        ...
        if self.use_spatial:
            mu_encoder, logvar = self.encoder(
                x.view(-1, self.input_dim), edge_index, edge_weight
            )
