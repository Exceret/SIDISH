...
def loss_DeepCox(pred, events, durations, weight=None, train=True):
    ...
    """
    if train:
        idx = durations.sort(descending=True)[
            1
...
class DEEPCOX:
    def __init__(
        ...
        self.model = deepCox(
            hidden=self.hidden, encoder=self.encoder, dropout=self.dropout
        )
    ...
    def train(self, epochs):
        ...
        for epoch in tqdm(range(epochs)):
            for x, y in self.train_loader:
                ...
                loss = loss_DeepCox(pred, events, days, w).view(
                    1,
                )
    ...
    def get_test_loss(self, test_loader):
        ...
        with torch.no_grad():
            ...
            loss_test = loss_DeepCox(
                pred_test, events_test, days_test, train=False
            ).view(
                1,
            )
        return loss_test
    ...
    def get_test_ci(self, test_loader):
        ...
        with torch.no_grad():
            ...
            ci_test = concordance_index(
                days_test.cpu().numpy(), out, events_test.cpu().numpy()
            )
        return ci_test
