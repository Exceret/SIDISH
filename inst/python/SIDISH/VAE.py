...
class VAE:
    def __init__(
        self, epochs, adata, z_dim, layer_dims, lr, dropout, device, seed, gcn_dims=None
    ):
    ...
    def initialize(
        ...
        if self.use_spatial and spatial_graph is not None:
            ...
            # Non-spatial
            model = architecture(
                self.imput_dim, self.z_dim, self.layer_dims, self.seed, self.dropout
            )
        ...
        if type == "Dense":
            ...
            # ! CHANGE HERE
            data_list = [np.array(self.adata.X.toarray()), self.W]
            data_list_total = [np.array(self.adata.X.toarray()), self.W]
            data_list = [torch.from_numpy(d).float() for d in data_list] 
        ...
        self.train_loader = DataLoader(
            dataset, batch_size=batch_size, **kwargs, drop_last=True
        )

        ## Setup mini-batching for the full dataset (prediction)
        data_list_total = [
            torch.from_numpy(np.array(d)).type(torch.float) for d in data_list_total
        ]
        total_dataset = TensorDataset(
            data_list_total[0].float(), data_list_total[1].float()
        )
        kwargs = {"num_workers": num_workers, "pin_memory": True}
        self.total_loader = DataLoader(
            total_dataset, batch_size=self.adata.X.shape[0], **kwargs
        )
        ...
        if self.use_spatial and spatial_graph is not None:
            ...
            self.train_loader_spatial = NeighborLoader(
                self.graph_data,
        ...
        for epoch in range(self.epochs):
            ...
            if self.use_spatial and self.use_neighbor_sampling:
                ...
                for batch in self.train_loader_spatial:
                    ...
                    mu_decoder, dropout_logits, mu_encoder, logvar = self.model(
                        batch.x, batch.edge_index, batch.edge_attr
                    )
                    loss = self.model.loss_function(
                        batch.x, batch.y, mu_decoder, dropout_logits, mu_encoder, logvar
                    )
                    loss.backward()
                ...
                for x, y in self.train_loader:
                    ...
                    loss = self.model.loss_function(
                        x, y_, mu_decoder, dropout_logits, mu_encoder, logvar
                    )
                    loss.backward()
                    epoch_loss += loss.item() * batch_size
                    samples_processed += batch_size
            ...
            print(
                "[epoch %03d]  average training loss: %.4f" % (epoch, self.epoch_loss)
            )
