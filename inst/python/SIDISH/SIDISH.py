...
from SIDISH.Utils import (
    get_spatial_graph_from_adata,
)  # Build spatial graph (for spatial-VAE)
...
from SIDISH.gene_perturbation_utils import (
    GenePerturbationUtils,
)  # Gene KO / network-based adj.
...
def process_Data(
    X: np.ndarray, Y: np.ndarray, test_size: float, batch_size: int, seed: int
) -> tuple:
    """
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        X, Y, test_size=test_size, random_state=seed, stratify=Y[:, 1]
    )
...
class SIDISH:
    ...
    def __init__(
        ...
        if self.use_spatial_graph and "spatial" in self.adata.obsm:
            edge_index, edge_weight = get_spatial_graph_from_adata(
                self.adata, spatial_key="spatial", method="knn", k=self.k_neighbors
            )
            self.spatial_graph = (edge_index, edge_weight)
            print(
                "SIDISH Spatial graph constructed using k-NN with k =", self.k_neighbors
            )
    ...
    def train(
        ...
        num_workers: int = 0,
        show: bool = True,
        distribution_fit: Literal["default", "fitted"] = "default",
    ) -> sc.AnnData:
        """
        ...
        self.test_loader = DataLoader(
            test_dataset, batch_size=self.X_test.shape[0], shuffle=False
        )
        ...
        if self.use_spatial_graph and self.spatial_graph is not None:
            ...
            self.vae.initialize(
                self.adata, self.W_matrix, self.batch_size, self.type, self.num_workers
            )

        # Initial training of VAE in iteration 1
        print(
            "########################################## ITERATION 1 OUT OF {} ##########################################".format(
                iterations
            )
        )
        self.vae.train()
        ...
        for i in range(iterations):
            ...
            self.W_temp = utils.getWeightMatrix(
                self.adata_, self.seed, self.steepness, self.type
            )
            ...
            print(
                "########################################## Saving Weight Matrix at Iteration {} ##########################################".format(
                    i
                )
            )
            ...
            if i == (iterations - 1):
                ...
                # Save VAE for iterative process
                torch.save(
                    self.vae.model.state_dict(), "{}vae_transfer".format(self.path)
                )
    ...
    def plotUMAP(
        ...
        sc.pp.neighbors(
            self.adata, n_neighbors=30, use_rep="latent", random_state=self.seed
        )
    ...
    def get_percentille(self, percentile):
        self.percentile = percentile
        self.percentile_cells = utils.get_threshold(
            self.adata, self.deepCox_model.model, self.percentile, self.device
        )
    ...
    def get_MarkerGenes(
        self, logfc_threshold=1.5, pval_threshold=0.05, method="wilcoxon", group="h"
    ):
        """
        ...
        ranked_genes_df = sc.get.rank_genes_groups_df(
            self.adata, group=group, key=f"SIDISH_deg"
        )
        ...
        self.upregulated_genes = ranked_genes_df[
            ...
            & (ranked_genes_df["pvals"] < pval_threshold)
        ]["names"].values
    ...
    def analyze_perturbation_effects(self):
        ...
        for gene, data in tqdm(
            zip(self.genes, self.optimized_results), total=len(self.genes), desc="Stats"
        ):
            adata_p = self.annotateCells(
                data, self.percentile_cells, mode="no", perturbation=True
            )

            h_to_b = (
                (self.adata.obs.SIDISH == "h") & (adata_p.obs.SIDISH == "b")
            ).sum()
            b_to_h = (
                (self.adata.obs.SIDISH == "b") & (adata_p.obs.SIDISH == "h")
            ).sum()
            ...
            if flips:  # avoid 0-trials edge case
                p_flip = binomtest(
                    k=h_to_b, n=flips, p=0.5, alternative="greater"
                ).pvalue
            else:
                p_flip = 1.0
            ...
            self.b_dict[gene] = (
                (self.adata.obs["SIDISH"] == "b") & (adata_p.obs["SIDISH"] == "b")
            ).sum()
            self.h_dict[gene] = (
                (self.adata.obs["SIDISH"] == "h") & (adata_p.obs["SIDISH"] == "h")
            ).sum()
    ...
    def run_Perturbation(self, n_jobs: int = 4) -> tuple:
        ...
        self.percent_change, self.delta_change, self.p_flip, self.p_score = (
            self.analyze_perturbation_effects()
        )
    ...
    def plot_KM(
        ...
        coef = (
            cph.summary.T.filter(self.upregulated_genes).iloc[0].values.reshape(-1, 1)
        )
        ...
        risk_group = np.where(
            risk_scores >= median_risk, high_risk_label, background_label
        )
    ...
    def plot_top_perturbed_genes(self, gene_data, top_n=20):
        ...
        self.top_genes = dict(
            sorted(gene_data.items(), key=lambda x: x[1], reverse=True)[:top_n]
        )

        # Plotting
        plt.figure(figsize=(8, 8))
        plt.barh(
            list(self.top_genes.keys()), list(self.top_genes.values()), color="brown"
        )
    ...
    def plot_perturbation_UMAP_default(
        self, genes_of_interest, resolution=None, celltype=True, threshold=0.8
    ):
        """
        ...
        fig, axs = plt.subplots(
            n_rows, n_cols, figsize=(8 * n_cols, 4.5 * n_rows), squeeze=False
        )
        axs = axs.flatten()
        ...
        for gene, ax in zip(genes_of_interest, axs):
            ...
            percentage_change = (
                (h_to_b - b_to_h) / (self.adata.obs.SIDISH == "h").values.sum()
            ) * 100
            ...
            umap_combined = pd.DataFrame(
                self.adata.obsm["X_umap"], columns=["UMAP1", "UMAP2"]
            )
            ...
            umap_combined["status"] = "{}".format(
                ((self.adata.obs.SIDISH == "b") & (adata_p.obs.SIDISH == "b")).sum()
            )

            umap_combined.loc[
                (umap_combined["risk"] == "h") & (adata_p.obs["SIDISH"] == "h"),
                "status",
            ] = "{}".format(
                ((self.adata.obs.SIDISH == "h") & (adata_p.obs.SIDISH == "h")).sum()
            )
            umap_combined.loc[
                (umap_combined["risk"] == "h") & (adata_p.obs["SIDISH"] == "b"),
                "status",
            ] = "{}".format(
                ((self.adata.obs.SIDISH == "h") & (adata_p.obs.SIDISH == "b")).sum()
            )
            umap_combined.loc[
                (umap_combined["risk"] == "b") & (adata_p.obs["SIDISH"] == "h"),
                "status",
            ] = "{}".format(
                ((self.adata.obs.SIDISH == "b") & (adata_p.obs.SIDISH == "h")).sum()
            )
    ...
    def plot_perturbation_UMAP_differential(
        self, genes_of_interest, resolution=None, celltype=True, threshold=0.8
    ):
        """
        Generates UMAP visualizations for specified genes after in-silico perturbation.
        ...
        for gene, ax in zip(genes_of_interest, axs):
            ...
            adata_p = self.annotateCells(
                adata_p, self.percentile_cells, mode="no", perturbation=True
            )
    ...
    def run_double_Perturbation(self, genes, top_n=20, threshold=0.8):
        ...
        pvalue_df_flip = pd.DataFrame(
            list(corrected_pvalue_dict_flip.items()), columns=["Gene", "Pvalue"]
        )
        ...
        self.percentage_df_flip = pd.DataFrame(
            [self.percent_change], index=None
        ).T.reset_index()
        self.percentage_df_flip.columns = ["Genes", "Scores"]
        self.percentage_df_flip.sort_values(
        ...
        for combination in tqdm(all_combinations[:]):
            ...
            self.percent_change_double[
                "{}+{}".format(combination[0], combination[1])
            ] = ((h_to_b - b_to_h) / n_hcells) * 100
            ...
            self.h_to_b_dict_double["{}+{}".format(combination[0], combination[1])] = (
                h_to_b
            )
            self.b_to_h_dict_double["{}+{}".format(combination[0], combination[1])] = (
                b_to_h
            )
            self.b_dict_double["{}+{}".format(combination[0], combination[1])] = (
                (self.adata.obs["SIDISH"] == "b") & (adata_p.obs["SIDISH"] == "b")
            ).sum()
            self.h_dict_double["{}+{}".format(combination[0], combination[1])] = (
                (self.adata.obs["SIDISH"] == "h") & (adata_p.obs["SIDISH"] == "h")
            ).sum()
    ...
    def run_double_Perturbation_score(self, genes, top_n=20, threshold=0.8):
        ...
        pvalue_df_score = pd.DataFrame(
            list(corrected_pvalue_dict_score.items()), columns=["Gene", "Pvalue"]
        )
        ...
        self.percentage_df_score = pd.DataFrame(
            [self.delta_change], index=None
        ).T.reset_index()
        self.percentage_df_score.columns = ["Genes", "Scores"]
        self.percentage_df_score.sort_values(
            by=["Scores"], ascending=False, inplace=True
        )
        ...
        for combination in tqdm(all_combinations[:]):
            ...
            self.delta_change_double["{}+{}".format(combination[0], combination[1])] = (
                delta_double.mean()
            )
            _, p_score_ = wilcoxon(delta_double, alternative="greater")
            self.p_score_double["{}+{}".format(combination[0], combination[1])] = (
                p_score_
            )
    ...
    def plot_double_Perturbation_Heatmap(self, percentage_double_dict, top_n=20):
        ...
        percent_change_df = pd.DataFrame(
            list(self.percent_change.items()), columns=["Genes", "Scores"]
        )
        ...
        heatmap_data = pd.DataFrame(
            [
                {"Gene1": k.split("+")[0], "Gene2": k.split("+")[1], "Value": v}
        ...
        heatmap_matrix = heatmap_data.pivot(
            index="Gene1", columns="Gene2", values="Value"
        )
        ...
        fig, (ax1, ax2) = plt.subplots(
            1, 2, gridspec_kw={"width_ratios": [1, 15]}, figsize=(10, 8)
        )  # Reduce width for 1D heatmap
