...
def extractFeature(adata, type="Normal"):
    ...
    train_X, test_X, train_y, test_y = train_test_split(
        C, risk, test_size=0.2, stratify=risk, random_state=42
    )
    ...
    class_weights = compute_class_weight(
        class_weight="balanced", classes=np.array([0, 1]), y=train_y
    )
    ...
    model = RandomForestClassifier(
        criterion="log_loss", n_jobs=-1, class_weight=class_weight_dict, random_state=42
    ).fit(train_X, train_y)
...
def create_spatial_graph(
    spatial_coords, method="knn", k=5, radius=None, include_self=False
):
    """
...
def get_spatial_graph_from_adata(
    adata, spatial_key="spatial", method="knn", k=5, radius=None, include_self=False
):
    """
...
def create_pytorch_geometric_data(adata, spatial_key="spatial", method="knn", k=5):
    ...
    edge_index, edge_attr = get_spatial_graph_from_adata(
        adata, spatial_key=spatial_key, method=method, k=k
    )
...
class Utils:
    def getWeightVector(
        ...
        percentile: float,
        device: str,
        distribution: Literal["default", "fitted"] = "default",
        dist=None,
    ):
        ...
        if distribution == "default":
            ...
            if dist is not None:
                ...
                ranked["rank_sum"] = ranked[["r_aic", "r_bic", "r_ks", "r_r2"]].sum(
                    axis=1
                )

                # Pick best by rank_sum, tie-break by AIC
                best_idx = ranked.sort_values(
                    ["rank_sum", "AIC"], ascending=[True, True]
                ).index[0]
                ...
                percentile_patients = DISTROS[best_hybrid].ppf(
                    percentile, *params_patients
                )
        ...
        return (
            torch.FloatTensor(val_p),
            adata,
            percentile_cells,
            percentile_cells.max(),
            percentile_cells.min(),
            returned_dist,
        )

    def annotateCells(
        adata, model, percentile_cells, device, percentile, mode, perturbation=False
    ):
    ...
    def getWeightMatrix(adata, seed, steepness=100, type="Normal"):
        ...
        explainer = shap.Explainer(
            model, train_X, seed=seed, feature_names=adata.to_df().columns
        )
        shap_values = explainer(test_X, check_additivity=False)
        shap_values_class_1 = shap_values.values[:, :, 0]
        shap_values = pd.DataFrame(
            copy.deepcopy(shap_values_class_1), columns=adata.to_df().columns
        )
