...
def process_Data(X: np.ndarray, Y: np.ndarray, test_size: float,
    ...
    data_list = [
        torch.from_numpy(np.array(d)).type(torch.float) for d in data_list
    ]
...
def map_event_column(val):
    ...
    raise ValueError(
        f"Invalid survival event found: '{val}'. "
        f"The status column must only contain 'Dead', 'Alive', 1, or 0.")
...
def preprocess(adata,
    ...
    if processed == False:
        ...
        data = bulk.copy(
        )  # assumes first two columns are metadata (survival days, event)
        bulk_genes = data.columns.to_numpy()

        if 'gene_ids' in adata.var.columns:
            sc_gene = adata.var['gene_ids'].astype(str).values
        else:
            sc_gene = adata.var_names.astype(str).values

        inter = np.intersect1d(bulk_genes, sc_gene)
        if inter.size == 0:
            raise ValueError(
                "No overlapping genes between bulk and scRNA-seq.")

        ## Keeping intersection between scRNA-seq and bulk
        if 'gene_ids' in adata.var.columns:
            adata = adata[:,
                          adata.var['gene_ids'].astype(str).isin(inter)].copy(
                          )
            subset = subset[:, subset.var['gene_ids'].astype(str).
                            isin(inter)].copy()
        else:
        ...
        if celltype_name in subset.obs.columns:
            subset.obs.rename(columns={celltype_name: "celltype_major"},
                              inplace=True)
        ...
        if batch_correction:
            ...
            if patient_id not in subset.obs.columns:
                subset.obs[patient_id] = subset.obs_names.str.split(
                    '_').str[-1]
        ...
        if celltype_name in adata.obs.columns:
            adata.obs.rename(columns={celltype_name: "celltype_major"},
                             inplace=True)
...
class SIDISH:
    ...
    def __init__(self,
        ...
        if self.use_spatial_graph and "spatial" in self.adata.obsm:
            edge_index, edge_weight = get_spatial_graph_from_adata(
                self.adata,
                spatial_key="spatial",
                method="knn",
                k=self.k_neighbors)
            self.spatial_graph = (edge_index, edge_weight)
            print("SIDISH Spatial graph constructed using k-NN with k =",
                  self.k_neighbors)
    ...
    def init_Phase2(self, epochs: int, hidden: int, lr: float, dropout: float,
                    test_size: float, batch_size_bulk: int) -> None:
        """
        ...
        self.X_train, self.X_test, self.y_train, self.y_test = process_Data(
            self.X, self.Y, test_size, batch_size_bulk, self.seed)
    ...
    def train(
        ...
        num_workers: int = 0,
        show: bool = True,
        distribution_fit: Literal["default",
                                  "fitted"] = "default") -> sc.AnnData:
        """
        ...
        if self.use_spatial_graph and self.spatial_graph is not None:
            print(
                "########################################## Using Spatial Graph in VAE ##########################################"
            )
            self.vae = VAE(epochs=self.epochs_1,
                           adata=self.adata,
                           z_dim=self.latent_size,
                           layer_dims=self.layer_dims,
                           lr=self.lr_1,
                           dropout=self.dropout_1,
                           device=self.device,
                           seed=self.seed,
                           gcn_dims=[32, self.latent_size])
            self.vae.initialize(self.adata,
                                W=self.W_matrix,
                                batch_size=self.batch_size,
                                num_workers=self.num_workers,
                                spatial_graph=self.spatial_graph,
                                num_neighbors=self.k_neighbors)

        else:
            print(
                "########################################## Using Dense VAE ##########################################"
            )
            self.vae = VAE(self.epochs_1, self.adata, self.latent_size,
                           self.layer_dims, self.lr_1, self.dropout_1,
                           self.device, self.seed)
            self.vae.initialize(self.adata, self.W_matrix, self.batch_size,
                                self.type, self.num_workers)
        ...
        torch.save(self.vae.model.state_dict(),
                   "{}vae_transfer".format(self.path))
        ...
        for i in range(iterations):
            ...
            self.deepCox_model = DeepCox(self.X_train, self.y_train,
                                         self.W_vector, self.hidden,
                                         self.encoder, self.device,
                                         self.batch_size, self.seed, self.lr_2,
                                         self.dropout_2)
            ...
            self.test_loss_Cox.append(
                self.deepCox_model.get_test_loss(self.test_loader))
            self.test_ci_Cox.append(
                self.deepCox_model.get_test_ci(test_loader=self.test_loader))
            ...
            if i == 0:
                self.scores, self.adata_, self.percentile_cells, self.cells_max, self.cells_min, returned_dist = utils.getWeightVector(
                    patients_data, self.vae.adata, self.deepCox_model.model,
                    self.percentile, self.device, distribution_fit)
                self.dist = returned_dist
            else:
                if distribution_fit == 'fitted':
                    self.scores, self.adata_, self.percentile_cells, self.cells_max, self.cells_min, returned_dist = utils.getWeightVector(
                        patients_data, self.vae.adata,
                        self.deepCox_model.model, self.percentile, self.device,
                        distribution_fit, self.dist)
                else:
                    self.scores, self.adata_, self.percentile_cells, self.cells_max, self.cells_min, returned_dist = utils.getWeightVector(
                        patients_data, self.vae.adata,
                        self.deepCox_model.model, self.percentile, self.device,
            ...
            self.W_temp = utils.getWeightMatrix(self.adata_, self.seed,
                                                self.steepness, self.type)
            ...
            print(
                "########################################## Saving Weight Matrix at Iteration {} ##########################################"
                .format(i))
            pd.DataFrame(self.W_matrix).to_csv("{}W_matrix_{}.csv".format(
                self.path, i))
            ...
            if i == (iterations - 1):
                ...
                print(
                    "########################################## ITERATION {} OUT OF {} ##########################################"
                    .format(i + 2, iterations))
                if self.use_spatial_graph and self.spatial_graph is not None:
                    self.vae = VAE(epochs=self.epochs_3,
                                   adata=self.adata,
                                   z_dim=self.latent_size,
                                   layer_dims=self.layer_dims,
                                   lr=self.lr_3,
                    ...
                    self.vae.model.load_state_dict(
                        torch.load("{}vae_transfer".format(self.path)))

                else:
                    self.vae = VAE(self.epochs_3, self.adata, self.latent_size,
                                   self.layer_dims, self.lr_3, self.dropout_1,
                                   self.device, self.seed)
                    self.vae.initialize(self.adata, self.W_matrix,
                                        self.batch_size, self.type,
                                        self.num_workers)
                    self.vae.model.load_state_dict(
                        torch.load("{}vae_transfer".format(self.path)))
        ...
        torch.save(self.deepCox_model.model.state_dict(),
                   "{}deepCox".format(self.path))
    ...
    def plotUMAP(self,
        ...
        self.adata.obs["SIDISH_"] = [
            "High-Risk Cells ({})".format(h)
            if i == "h" else "Background Cells ({})".format(b)
            for i in self.adata.obs.SIDISH
        ]
        self.adata.uns["SIDISH__colors"] = np.array(["grey", "red"],
                                                    dtype="object")

        self.adata.obs["SIDISH__"] = [
            "High-Risk" if i == "h" else "Background"
            for i in self.adata.obs.SIDISH
        ]
        self.adata.uns["SIDISH___colors"] = np.array(["grey", "red"],
                                                     dtype="object")
    ...
    def annotateCells(self,
                      test_adata,
                      percentile_cells,
                      mode,
                      perturbation=False):

        adata_new = utils.annotateCells(test_adata,
                                        self.deepCox_model.model,
                                        percentile_cells,
                                        self.device,
                                        self.percentile,
                                        mode=mode,
    ...
    def reload(self, path, num_workers=0):
        ...
        self.deepCox_model = DeepCox(self.X_train, self.y_train, self.W_vector,
                                     self.hidden, self.encoder, self.device,
                                     self.batch_size, self.seed, self.lr_2,
                                     self.dropout_2)
        self.deepCox_model.model.load_state_dict(
            torch.load("{}deepCox".format(self.path)))

        print("✅ Reload complete – VAE and DeepCox restored")

    def get_percentille(self, percentile):
        self.percentile = percentile
        self.percentile_cells = utils.get_threshold(self.adata,
                                                    self.deepCox_model.model,
                                                    self.percentile,
                                                    self.device)
    ...
    def get_embedding(self, n_neighbors=30, resolution=None, celltype=True):
        if celltype and resolution is not None:
            raise ValueError(
                "Resolution should not be provided when celltype=True.")
    ...
    def plot_HighRisk_UMAP(self, size=10, resolution=None, celltype=True):
        self.adata = self.get_embedding(resolution=resolution,
                                        celltype=celltype)
        self.set_adata()
    ...
    def get_MarkerGenes(self,
        ...
        if method not in supported_methods:
            raise ValueError(
                f"Unsupported method. Choose from {supported_methods}.")
        ...
        sc.tl.rank_genes_groups(self.adata,
                                ...
                                groups=[group],
                                method=method,
                                key_added=f"SIDISH_deg")

        # Extract ranked genes for the specified group
        ranked_genes_df = sc.get.rank_genes_groups_df(self.adata,
                                                      group=group,
                                                      key=f"SIDISH_deg")

        # Filter for upregulated and downregulated genes
        self.upregulated_genes = ranked_genes_df[
            (ranked_genes_df["logfoldchanges"] > logfc_threshold)
            & (ranked_genes_df["pvals"] < pval_threshold)]["names"].values

        self.downregulated_genes = ranked_genes_df[
            (ranked_genes_df["logfoldchanges"] < -logfc_threshold)
            & (ranked_genes_df["pvals"] < pval_threshold)]["names"].values
    ...
    def analyze_perturbation_effects(self):
        ...
        for gene, data in tqdm(zip(self.genes, self.optimized_results),
                               total=len(self.genes),
                               desc="Stats"):

            adata_p = self.annotateCells(data,
                                         self.percentile_cells,
                                         mode="no",
                                         perturbation=True)

            h_to_b = ((self.adata.obs.SIDISH == "h") &
                      (adata_p.obs.SIDISH == "b")).sum()
            b_to_h = ((self.adata.obs.SIDISH == "b") &
                      (adata_p.obs.SIDISH == "h")).sum()
            ...
            self.b_dict[gene] = ((self.adata.obs["SIDISH"] == "b") &
                                 (adata_p.obs["SIDISH"] == "b")).sum()
            self.h_dict[gene] = ((self.adata.obs["SIDISH"] == "h") &
                                 (adata_p.obs["SIDISH"] == "h")).sum()
    ...
    def run_Perturbation(self, n_jobs: int = 4) -> tuple:
        ...
        self.optimized_results = perturbation.run_parallel_processing(
            self.adata, n_jobs=4)
    ...
    def plot_KM(self,
        ...
        coef = cph.summary.T.filter(
            self.upregulated_genes).iloc[0].values.reshape(-1, 1)
        expression = result.iloc[:, :-2].values
        risk_scores = np.dot(expression, coef)

        # Classify Patients into High/Low Risk
        median_risk = np.median(risk_scores)
        risk_group = np.where(risk_scores >= median_risk, high_risk_label,
                              background_label)
        ...
        kmf.fit(high_risk_group["duration"],
        ...
        kmf.fit(low_risk_group["duration"],
    ...
    def plot_top_perturbed_genes(self, gene_data, top_n=20):
        ...
        self.top_genes = dict(
            sorted(gene_data.items(), key=lambda x: x[1],
                   reverse=True)[:top_n])

        # Plotting
        plt.figure(figsize=(8, 8))
        plt.barh(list(self.top_genes.keys()),
                 list(self.top_genes.values()),
                 color='brown')
        plt.xlabel('% Reduction of High-Risk Cells')
        plt.title(
            f'Top {top_n} Genes by Reduction in High-Risk Cells After Perturbation'
        )
        plt.gca().invert_yaxis(
        )  # Invert y-axis to show the highest reduction on top
        plt.show()

    def plot_perturbation_UMAP_default(self,
                                       genes_of_interest,
                                       resolution=None,
                                       celltype=True,
                                       threshold=0.8):
        ...
        for gene, ax in zip(genes_of_interest, axs):
            ...
            direct_neighbors, indirect_neighbors = self.ppi_handler.get_neighbors(
                gene)
            ...
            if not network_df.empty:
                adata_p = GenePerturbationUtils.adjust_expression(
                    self.adata, gene, network_df)
            else:
                adata_p.X = GenePerturbationUtils.knockout_gene(
                    self.adata, gene).tocsr()
            ...
            percentage_change = (
                (h_to_b - b_to_h) /
                (self.adata.obs.SIDISH == "h").values.sum()) * 100
            ...
            umap_combined = pd.DataFrame(self.adata.obsm["X_umap"],
                                         columns=["UMAP1", "UMAP2"])
            umap_combined.index = self.adata.obs.index.values
            umap_combined['risk'] = self.adata.obs['SIDISH'].values

            umap_combined['status'] = '{}'.format(
                ((self.adata.obs.SIDISH == "b") &
                 (adata_p.obs.SIDISH == "b")).sum())

            umap_combined.loc[(umap_combined['risk'] == 'h') &
                              (adata_p.obs['SIDISH'] == 'h'),
                              'status'] = '{}'.format(
                                  ((self.adata.obs.SIDISH == "h") &
                                   (adata_p.obs.SIDISH == "h")).sum())
            umap_combined.loc[(umap_combined['risk'] == 'h') &
                              (adata_p.obs['SIDISH'] == 'b'),
                              'status'] = '{}'.format(
                                  ((self.adata.obs.SIDISH == "h") &
                                   (adata_p.obs.SIDISH == "b")).sum())
            umap_combined.loc[(umap_combined['risk'] == 'b') &
                              (adata_p.obs['SIDISH'] == 'h'),
                              'status'] = '{}'.format(
                                  ((self.adata.obs.SIDISH == "b") &
                                   (adata_p.obs.SIDISH == "h")).sum())
            ...
            palette = {
                '{}'.format(((self.adata.obs.SIDISH == "b") & (adata_p.obs.SIDISH == "b")).sum(
                            )):
                'darkgray',
                '{}'.format(((self.adata.obs.SIDISH == "h") & (adata_p.obs.SIDISH == "h")).sum(
                            )):
                'red',
                '{}'.format(((self.adata.obs.SIDISH == "h") & (adata_p.obs.SIDISH == "b")).sum(
                            )):
                '#3DB1EA',
                '{}'.format(((self.adata.obs.SIDISH == "b") & (adata_p.obs.SIDISH == "h")).sum(
                            )):
                'purple',
            ...
            ax.set_title("In-Silico Knockout of {}".format(gene),
                         fontsize=12,
                         y=0.96)
    ...
    def plot_perturbation_UMAP_differential(self,
                                            genes_of_interest,
                                            resolution=None,
                                            celltype=True,
                                            threshold=0.8):
        ...
        for gene, ax in zip(genes_of_interest, axs):
            ...
            adata_p = self.annotateCells(adata_p,
                                         self.percentile_cells,
                                         mode="no",
                                         perturbation=True)
            ...
            ax.set_title(
                "In silico Knockout of {} (Change in score)".format(gene),
                fontsize=12,
                y=0.96)
    ...
    def run_double_Perturbation(self, genes, top_n=20, threshold=0.8):
        ...
        corrected_pvals_flip = multipletests(pvals_flip,
                                             alpha=0.05,
                                             method='fdr_bh')

        corrected_pvalue_dict_flip = dict(
            zip(self.p_flip.keys(), corrected_pvals_flip[1]))

        pvalue_df_flip = pd.DataFrame(list(corrected_pvalue_dict_flip.items()),
                                      columns=["Gene", "Pvalue"])
        ...
        self.percentage_df_flip = pd.DataFrame([self.percent_change],
                                               index=None).T.reset_index()
        self.percentage_df_flip.columns = ["Genes", "Scores"]
        self.percentage_df_flip.sort_values(by=["Scores"],
                                            ascending=False,
                                            inplace=True)
        ...
        all_combinations = list(
            itertools.permutations(self.top_genes[:top_n], 2))
        n_hcells = (self.adata.obs.SIDISH == "h").sum()
        ...
        for combination in tqdm(all_combinations[:]):
            ...
            self.percent_change_double["{}+{}".format(
                combination[0],
                combination[1])] = ((h_to_b - b_to_h) / n_hcells) * 100

            # --- 1  one-sided sign test on label flips ---
            flips = h_to_b + b_to_h
            if flips:  # avoid 0-trials edge case
                p_flip_ = binomtest(k=h_to_b,
                                    n=flips,
                                    p=0.5,
                                    alternative="greater").pvalue
            else:
                p_flip_ = 1.0
            self.p_flip_double["{}+{}".format(combination[0],
                                              combination[1])] = p_flip_

            self.h_to_b_dict_double["{}+{}".format(combination[0],
                                                   combination[1])] = h_to_b
            self.b_to_h_dict_double["{}+{}".format(combination[0],
                                                   combination[1])] = b_to_h
            self.b_dict_double["{}+{}".format(
                combination[0],
                combination[1])] = ((self.adata.obs["SIDISH"] == "b") &
                                    (adata_p.obs["SIDISH"] == "b")).sum()
            self.h_dict_double["{}+{}".format(
                combination[0],
                combination[1])] = ((self.adata.obs["SIDISH"] == "h") &
                                    (adata_p.obs["SIDISH"] == "h")).sum()
    ...
    def run_double_Perturbation_score(self, genes, top_n=20, threshold=0.8):
        ...
        corrected_pvals_score = multipletests(pvals_score,
                                              alpha=0.05,
                                              method='fdr_bh')

        corrected_pvalue_dict_score = dict(
            zip(self.p_score.keys(), corrected_pvals_score[1]))

        pvalue_df_score = pd.DataFrame(list(
            corrected_pvalue_dict_score.items()),
                                       columns=["Gene", "Pvalue"])
        ...
        self.percentage_df_score = pd.DataFrame([self.delta_change],
                                                index=None).T.reset_index()
        self.percentage_df_score.columns = ["Genes", "Scores"]
        self.percentage_df_score.sort_values(by=["Scores"],
                                             ascending=False,
                                             inplace=True)
        ...
        for combination in tqdm(all_combinations[:]):
            ...
            self.delta_change_double["{}+{}".format(
                combination[0], combination[1])] = delta_double.mean()
            _, p_score_ = wilcoxon(delta_double, alternative="greater")
            self.p_score_double["{}+{}".format(combination[0],
                                               combination[1])] = p_score_

        return self.delta_change_double, self.p_score_double

    def plot_double_Perturbation_Heatmap(self,
                                         percentage_double_dict,
                                         top_n=20):

        percent_change_df = pd.DataFrame(list(self.percent_change.items()),
                                         columns=["Genes", "Scores"])
        percent_change_df.sort_values(by='Scores',
                                      ascending=False,
                                      inplace=True)
        ...
        heatmap_matrix = heatmap_data.pivot(index='Gene1',
                                            columns='Gene2',
                                            values='Value')
        ...
        fig, (ax1,
              ax2) = plt.subplots(1,
                                  ...
                                  figsize=(10,
                                           8))  # Reduce width for 1D heatmap

        # Plot 1D heatmap on the first axis (ax1)
        sns.heatmap(df[['Scores']],
                    cmap='Reds',
                    cbar=True,
                    yticklabels=df['Genes'],
                    xticklabels=False,
                    ax=ax1)
